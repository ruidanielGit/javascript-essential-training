...
const newStrapLength = (strapArray) => {
  ...
  strapArray.forEach((listElement) => {
    ...
    // Update the text of the lenght
    lengthForm.addEventListener("submit", (e) => {
      // stop form from realoading the page
      e.preventDefault();
...
const backpackList = backpackObjectArray.map((backpack) => {
  ...
  let buttonLid = backpackArticle.querySelector(".lid-toggle");
  let newArg = "The argument I want to pass to the callback function!";

  // Add event listener
  buttonLid.addEventListener("click", (event) => {
    lidToggle(event, buttonLid, newArg);
  });
